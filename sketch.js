...
// ------------------------------------------------------------
const COIN = {
  frameWidth: 2310, // 256px total / 8 frames
  frameHeight: 3000, // only one row, full sheet height
  numFrames: 9, // 8 frames of spin animation
  animSpeed: 20, // draw() frames per sprite frame (lower = faster)
  scale: 0.15, // scale up so the coin is visible on screen
};
...
// ------------------------------------------------------------
let coins = [{ x: 200, y: 150, frame: 0, frameTimer: 0 }];
...
// ============================================================
function preload() {
  coinSheet = loadImage("assets/images/test_two.png");
}
...
function drawCoins() {
  for (let i = 0; i < coins.length; i++) {
    ...
    image(
      coinSheet,
      coin.x,
      coin.y, // destination centre position
      dw,
      dh, // destination size (scaled)
      sx,
      sy, // source position on sheet
      COIN.frameWidth - 200, // source width  (one frame)
